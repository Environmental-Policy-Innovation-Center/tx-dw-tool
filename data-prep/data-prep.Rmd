...
# calculating population density in people/mile: 
sabs <- demo$census %>%
...
sabs <- sabs %>%
  ...
  select(pwsid, pws_name, city_name, area_miles, pop_density, estimate_total_pop,
         # grabbing all races
...
app_test_df <- analysis_keys %>%
  left_join(sabs) %>%
  relocate(pws_name:city_name, .after = pwsid) %>%
  relocate(pop_categories:method, .after = city_name)  %>%
